...
import { Upload, Map as MapIcon, Image as ImageIcon, AlertCircle, Loader2 } from "lucide-react";
...
const AppContent: React.FC = () => {
  ...
  const handleAnalyze = async () => {
    if (!incidentImage) {
      setError(t('error.upload_required'));
      return;
    }
    ...
    try {
      // Pass the current locale context to the API
      const result = await analyzeDisaster(incidentImage, mapImage, locale);
      setReport(result);
    } catch (err: any) {
        ...
        <div className="max-w-7xl mx-auto px-4 py-4 flex flex-col sm:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-2 cursor-pointer" onClick={resetForm}>
            ...
            <h1 className="text-2xl font-black tracking-tighter text-white">
              {t('header.title')}<span className="text-red-500">AI</span>
            </h1>
          </div>
          
          <div className="flex items-center gap-4">
             <LanguageSelector disabled={loading} />
             <div className="text-xs font-mono text-gray-500 hidden sm:block">{t('header.subtitle')}</div>
          </div>
        </div>
      </header>
        ...
        {!report && (
          <div className="max-w-2xl mx-auto space-y-8 animate-fade-in-up">
            <div className="text-center space-y-2 mb-8">
              <h2 className="text-3xl font-bold text-white">{t('upload.incident_label')}</h2>
              <p className="text-gray-400">{t('upload.incident_desc')}</p>
            </div>
              ...
              >
                ...
                {incidentImage ? (
                  <>
                    ...
                    <span className="text-xs text-gray-500 mt-1">{t('upload.photo_change')}</span>
                  </>
                ) : (
                  <>
                    ...
                    <h3 className="text-lg font-semibold text-white">{t('upload.photo_label')}</h3>
                    <p className="text-sm text-gray-500 mt-2">{t('upload.photo_required')}</p>
                  </>
                )}
              </div>
              ...
              >
                ...
                {mapImage ? (
                  <>
                    ...
                    <span className="text-xs text-gray-500 mt-1">{t('upload.map_change')}</span>
                  </>
                ) : (
                  <>
                    <div className="w-12 h-12 rounded-full bg-gray-800 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                    ...
                    <h3 className="text-lg font-semibold text-white">{t('upload.map_label')}</h3>
                    <p className="text-sm text-gray-500 mt-2">{t('upload.map_optional')}</p>
                  </>
                )}
              </div>
            </div>
        ...
        {loading && !report && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-950/80 backdrop-blur-sm pointer-events-none">
            <div className="text-center p-8">
              ...
              <p className="mt-4 text-xl font-bold text-white tracking-widest animate-pulse">{t('loading.assessing')}</p>
              <p className="text-sm text-gray-500 mt-2">{t('loading.processing')}</p>
            </div>
          </div>
        )}
      </main>
